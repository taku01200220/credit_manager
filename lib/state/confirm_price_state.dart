...
final confirmPriceProvider =
    StateNotifierProvider<ConfirmPriceNotifier, int>((ref) {
  return ConfirmPriceNotifier();
});

class ConfirmPriceNotifier extends StateNotifier<int> {
  ConfirmPriceNotifier() : super(0);

  void calcConfirmPrice(List<Event> event, String cardName, String closingDate) {
    var price = 0;
  ...
  DateTime isStartDate(String closingDate) {
    ...
    if (int.parse((DateTime.now().day).toString()) <= int.parse(closingDate)) {
      String str = '${(DateTime.now().year).toString()}-${(DateTime.now().month - 2).toString()}-${int.parse(closingDate) + 1}';
